...
let pyodideReadyPromise = (async () => {
  ...
  const optimizerResponse = await fetch(new URL("./python/optimizer.py?v=44", import.meta.url), { cache: "no-store" });
  ...
  const kyberResponse = await fetch(new URL("./python/kyber_data.py?v=44", import.meta.url), { cache: "no-store" });
  if (kyberResponse.ok) {
  ...
  const profilesResponse = await fetch(new URL("./python/optimizer_profiles.json?v=44", import.meta.url), { cache: "no-store" });
